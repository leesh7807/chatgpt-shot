...
export async function runService(): Promise<void> {
  ...
  server = createServer(async (req, res) => {
    ...
    if (req.url !== '/jobs' || req.method !== 'POST' || stopping) return json(res, stopping ? 503 : 404, { code: stopping ? 'SERVICE_STOPPING' : 'NOT_FOUND', message: 'Service is not accepting this request.' });
    ...
    req.once('aborted', clearBodyDeadline); req.once('close', clearBodyDeadline); req.setEncoding('utf8'); req.on('data', chunk => body += chunk); req.on('end', async () => {
      clearBodyDeadline();
      try {
        const input = JSON.parse(body); const prompt = input?.prompt; const fields = input && typeof input === 'object' && !Array.isArray(input) ? Object.keys(input) : [];
        if (!input || typeof input !== 'object' || Array.isArray(input) || fields.length !== 1 || fields[0] !== 'prompt' || typeof prompt !== 'string' || !prompt.trim()) fail('CONFIG_INVALID', 'jobs requires only one non-empty prompt.');
        ...
        try { const run = await startJob(store, databaseId, new ChatGPTBrowser(current.browserProfilePath), prompt, id, { acknowledgementMs: current.acknowledgementMs, signal: admission.signal }); active++; void run.completion.catch(() => {}).finally(() => { active--; }); return json(res, 200, { id }); }
