...
test('accepts only a positive acknowledgement timeout', () => {
  const prior = process.env.XDG_CONFIG_HOME; const directory = mkdtempSync(join(tmpdir(), 'chatgpt-shot-config-'));
  try {
    process.env.XDG_CONFIG_HOME = directory;
    const state = paths(); setConfigValue('CHATGPT_SHOT_ACKNOWLEDGEMENT_TIMEOUT_MS', '1', state);
    assert.equal(readConfigValues(state).CHATGPT_SHOT_ACKNOWLEDGEMENT_TIMEOUT_MS, '1');
